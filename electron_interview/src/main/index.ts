...
import { app, BrowserWindow, ipcMain, desktopCapturer, session, globalShortcut, Menu, screen, shell } from 'electron'
...
function resolveWhisperPaths(): WhisperPaths {
  ...
  if (envModel && envCli && envServer) {
    ...
    return { modelPath: envModel, cliBinaryPath: envCli, serverBinaryPath: envServer }
  }
  ...
  for (const base of candidates) {
    ...
    const cliBin = resolve(whisperDir, 'build', 'bin', 'Release', 'whisper-cli.exe')
    const serverBin = resolve(whisperDir, 'build', 'bin', 'Release', 'whisper-server.exe')
    if (existsSync(model)) {
      console.log('[whisper] Found model at:', model)
      console.log('[whisper] CLI binary:', existsSync(cliBin) ? cliBin : 'not found')
      console.log('[whisper] Server binary:', existsSync(serverBin) ? serverBin : 'not found')
      return {
  ...
  console.warn('[whisper] Could not find whisper.cpp model in any candidate dir')
  return {
...
const MIC_FALLBACK_ENABLED =
  process.env.AUDIO_MIC_FALLBACK === '1' || process.env.AUDIO_MIC_FALLBACK === 'true'
...
const sttConfig = {
  mode: (DEEPGRAM_API_KEY ? 'deepgram' : STT_MODE) as 'whisper' | 'parakeet' | 'deepgram',
  ...
  whisperInitialPrompt: process.env.STT_PROMPT || process.env.WHISPER_INITIAL_PROMPT || 'Interview Q&A. Technical terms: Node.js, JavaScript, TypeScript, React, Python, API, database, algorithm, system design, frontend, backend.',
  whisperNoSpeechThreshold: Number(process.env.WHISPER_NO_SPEECH_THRESHOLD) || 0.6,
...
type SnapPosition = 'top-left' | 'top-right' | 'bottom-left' | 'bottom-right' | 'left-half' | 'right-half'
...
ipcMain.on('history:append', (_event, qa: unknown) => {
  historyService.append(qa as { id: string; question: string; answer: string; timestamp: number })
})
...
ipcMain.on(IPC.SESSION_SET_PROJECT, (_event, projectId: string | null) => {
  orchestrator?.setActiveProject(typeof projectId === 'string' ? projectId : null)
})
...
ipcMain.on(IPC.WINDOW_SNAP, (_event, position: SnapPosition) => {
  ...
  let px = x
  let py = y
  if (position === 'top-right' || position === 'right-half' || position === 'bottom-right') {
...
ipcMain.on(IPC.WINDOW_TOGGLE_COMPACT, () => {
  ...
  const isCompact = bounds.width <= COMPACT_WIDTH && bounds.height <= COMPACT_HEIGHT
  win.setBounds({
    x: bounds.x,
    y: bounds.y,
...
async function createWindow(): Promise<void> {
  ...
  const sttDebug = process.env.STT_DEBUG === '1' || process.env.STT_DEBUG === 'true'
...
app.whenReady().then(async () => {
  ...
  session.defaultSession.setDisplayMediaRequestHandler(async (_request, callback) => {
    ...
    callback({
      audio: 'loopback',
      video: sources[0] ? { id: sources[0].id, name: sources[0].name } : undefined
    })
  })
